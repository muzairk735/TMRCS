...
public class Doctor extends Person implements Serializable, AppointmentViewerInterface {
    ...
    public Doctor(
        ...
        this.specialization    = validateAndNormalizeSpecialization(specialization);
        this.licenseNumber    = validateAndNormalizeLicense(licenseNumber);
        this.experienceYears  = validateExperienceYears(experienceYears);
        this.consultationFee  = validateConsultationFee(consultationFee);
    ...
    private static String validateAndNormalizeSpecialization(String s) {
    ...
    private static String validateAndNormalizeLicense(String l) {
        if (l == null || l.trim().isEmpty()) {
            throw new IllegalArgumentException("License number cannot be empty.");
        }
        String normalized = l.trim().toUpperCase();
        if (!normalized.matches("^[A-Za-z0-9\\-]{5,20}$")) {
    ...
    private static int validateExperienceYears(int y) {
        if (y < 0 || y > 70) {
    ...
    private static double validateConsultationFee(double f) {
        if (f < 0) {
    ...
    public void displayProfile() {
        int W = 38;
        System.out.println("\n╔" + "═".repeat(W + 2) + "╗");
        System.out.println("║" + UIHelper.center("DOCTOR PROFILE", W + 2) + "║");
        System.out.println("╠" + "═".repeat(W + 2) + "╣");
        UIHelper.printRow("Doctor ID",      userId,                     W);
        UIHelper.printRow("Name",           "Dr. " + name,              W);
        UIHelper.printRow("Specialization", specialization,             W);
        UIHelper.printRow("License",        licenseNumber,              W);
        UIHelper.printRow("Experience",     experienceYears + " years", W);
        UIHelper.printRow("Fee",            "Rs. " + consultationFee,  W);
        UIHelper.printRow("Email",          email,                      W);
        UIHelper.printRow("Phone",          phoneNumber,                W);
        if (totalRatings > 0) {
            UIHelper.printRow("Rating", String.format("%.1f/5.0 (%d reviews)", rating, totalRatings), W);
        }
    ...
    public void viewAppointments(String status) {
        ...
        System.out.println("\n╔════════════════════════════════════════╗");
        System.out.println("║  " + UIHelper.padRight(status.toUpperCase() + " APPOINTMENTS", 37) + "║");
        System.out.println("╚════════════════════════════════════════╝");
    ...
    public void runConsultationSession(Appointment appointment, Scanner scanner) {
        ...
        System.out.println("\n╔════════════════════════════════════════╗");
        System.out.println("║   CONSULTATION SESSION - " + UIHelper.padRight(mode, 14) + "║");
        System.out.println("╠════════════════════════════════════════╣");
        UIHelper.printRow("Patient", patient.getName(), 38);
        UIHelper.printRow("Doctor", "Dr. " + name, 38);
        String sym = appointment.getSymptoms();
        UIHelper.printRow("Symptoms", sym.length() > 28 ? sym.substring(0, 27) + "…" : sym, 38);
        System.out.println("╚════════════════════════════════════════╝\n");
    ...
    public void setSpecialization(String s) { this.specialization = validateAndNormalizeSpecialization(s); }

    public String getLicenseNumber() { return licenseNumber; }
    public void setLicenseNumber(String l) { this.licenseNumber = validateAndNormalizeLicense(l); }

    public int getExperienceYears() { return experienceYears; }
    public void setExperienceYears(int y) { this.experienceYears = validateExperienceYears(y); }

    public double getConsultationFee() { return consultationFee; }
    public void setConsultationFee(double f) { this.consultationFee = validateConsultationFee(f); }
